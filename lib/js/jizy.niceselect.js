...
(function(){ 
	...
	function hasClass(el, className) {
		if ( el && className ) return el.classList.contains(className);
		else return false;
	}

	function addClass(el, className) {
		if ( el && className ) return el.classList.add(className);
	}

	function removeClass(el, className) {
		if ( el && className ) return el.classList.remove(className);
	}
	...
	NiceSelect.prototype.renderDropdown = function() {
		...
		var className = attr(this.el, "class");
		if ( className ) classes.push(className); 
		if ( this.disabled ) classes.push("disabled"); 
		if ( this.multiple ) classes.push("has-multiple"); 
		
		let searchHtml = '<div class="searchbox"><input type="text" placeholder="'+this.config.searchPlaceholder+'" /></div>';

		var html = '<div class="'
			+ classes.join(" ")+'" tabindex="'
	...
	NiceSelect.prototype._renderSelectedItems = function() {
		if ( this.multiple ) {
			...
			if ( window.getComputedStyle(this.dropdown).width == 'auto' || this.selectedOptions.length < 2 ){
				...
				selectedHtml = selectedHtml == "" ? this.config.placeholder : selectedHtml;
			}
			else{
			...
			var html = this.selectedOptions.length > 0 ? this.selectedOptions[0].data.text : this.config.placeholder;
	...
	NiceSelect.prototype._bindSearchEvent = function() {
		var searchBox = this.dropdown.querySelector(".searchbox > input");
		if ( searchBox ){
	...
	NiceSelect.prototype._onClicked = function(e) {
		...
		if ( this.dropdown.classList.contains("open") ){
			var search = this.dropdown.querySelector(".searchbox > input");
			if ( search ){
				search.value = "";
				search.focus();
			}
	...
	NiceSelect.prototype._findNext = function(el) {
		if ( el ){
			...
			el = this.dropdown.querySelector("ul li.option");
		}
	...
	NiceSelect.prototype._findPrev = function(el) {
		if ( el ){
			...
			el = this.dropdown.querySelector("ul li.option:last-child");
		}
	...
	JiZy.NiceSelect = function(el, options) {
		// console.log(options);
